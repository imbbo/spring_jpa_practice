...
@Transactional // JPA 레파지토리는 트랜잭션 단위로 동작하기 때문에 작성을 꼭 해야함
public class PostService {
    ...
    public PostListResponseDTO getPosts(PageDTO dto) {
        ...
        return PostListResponseDTO.builder()
                ...
                .pageInfo(new PageResponseDTO(posts)) // 생성자에게 apge 정보가 담긴 객체를 그대로 전달
                .posts(detailList)
                .build();
            }
    ...
    public PostDetailResponseDTO insert(final PostCreateDTO dto)
        ...
        if (hashTags != null && hashTags.size() > 0) {
            hashTags.forEach(ht -> {
                ...
                // Post Entity는 DB에 save를 진행할 때 HashTag에 대한 내용을 갱신하지 않습ㄴ다.
                // HashTag Entity는 따로 save를 진행합니다.
                // HashTag는 연관관계의 주인이기 떄문에 save를 진행할 때 Post를 전달하기 떄문에
                // DB와 Entity와의 상태가 동일하지만, Post는 HashTag의 정보가 비어있는 상태입니다.
                // Post Entity에 연관관계 편의 메서드를 작성하여 HashTag의 내용을 동기화 해야
                // 추후에 진행되는 고정에서 문제가 발생하지 않습니다.(연관관계 주인이 아니기 떄문에)
