...
public class PostApiController {
    ...
    @Parameters(
            {
                    @Parameter(name = "작성자", description = "게시물의 작성자 이름을 쓰세요", example = "김뽀비", required = true),
    ...
    public ResponseEntity<?> update(
            @Validated @RequestBody PostModifyDTO dto, BindingResult result, HttpServletRequest reqeust) {

        log.info("/api/v1/posts {} - dto: {}"
                , reqeust.getMethod(), dto);

        ResponseEntity<List<FieldError>> fieldErrors = getValidatedResult(result);
        if (fieldErrors != null) return fieldErrors;

        PostDetailResponseDTO resonseDTO = postService.modify(dto);
